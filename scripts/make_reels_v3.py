"""Batch render enhanced reels for all plan dates.

Reads PLAN.json + facts_bank.yml to get fact year, headline, and story
for the data-driven overlays. Uses media_reel_v3.compose_reel().
"""
...
from contenthub.media_reel_v3 import compose_reel
...
def main(dry: bool = False):
    ...
    for entry in plan:
        ...
        log.info("Rendering %s  icon=%s  year=%d…", date, icon, year)
        compose_reel(slides, icon, out, cfg, plan_entry=enriched, seed=hash(date) % 10000)
        rendered += 1

    log.info("Done — rendered %d enhanced reels", rendered)
